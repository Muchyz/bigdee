...
function Navbar() {
  ...
  const links = ["Home","About","Services","Why Us","Clients","Contact"];
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 transition-all duration-300"
        ...
        <div className="hidden lg:flex items-center gap-8">
          {links.map((l) => (
            <button key={l} onClick={() => scrollTo(l.toLowerCase().replace(" ","-"))}
              className="text-sm font-medium text-gray-600 hover:text-red-600 transition-colors">{l}</button>
          ))}
      ...
      {open && (
        <div className="lg:hidden bg-white border-t border-gray-100 px-6 py-4 space-y-1">
          {links.map((l) => (
            <button key={l} onClick={() => { scrollTo(l.toLowerCase().replace(" ","-")); setOpen(false); }}
              className="block w-full text-left px-4 py-3 rounded-xl text-sm font-medium text-gray-700 hover:bg-red-50 hover:text-red-700 transition-colors">{l}</button>
          ))}
...
function Footer() {
  const links = {
    "Company": ["About Us", "Director Statement", "Careers", "News"],
    "Services": services.map(s => s.title),
    "Support": ["Contact Us", "Request a Quote", "FAQs", "Privacy Policy"],
  };
  return (
